...
from config.config import RECTIFY
...
def convert_video(video_file_path, calib_params, output_file_path):
    ...
    if not vidcap.isOpened():
        logger.error(f"Could not open the videofile {video_file_path}")
        return
    ...
    # Loop through frames
    for i in tqdm(np.arange(frame_count), desc="Frame rectification"):
        start_time = time.time()
        success, frame = vidcap.read()
        if not success:
            logger.error(f"Error in the middle of reading videofile {video_file_path} in frame {i}")
            return
        ...
        start_time = time.time()
        out.write(undistorted.astype(np.uint8))
        time_write += time.time() - start_time

    print(f"Time spent on vidcap.read(): {time_read:.4f} seconds")
    print(f"Time spent on cv2.remap(): {time_remap:.4f} seconds")
    print(f"Time spent on out.write(): {time_write:.4f} seconds")
    out.release()


def rectify_video(checkerboard_input_video_path=None, calibration_config_dir=None, output_path=None, input_video_path=None, verbose=False):
    """
    Main function to perform video rectification.

    :param checkerboard_input_video_path: Path to checkerboard video for calibration.
    :param calibration_config_dir: Path to existing calibration config directory.
    :param output_path: Path to output directory.
    :param input_video_path: Path to video to rectify.
    :param verbose: Enable verbose logging.
    """
    rect_path = _rectified_video_path(input_video_path, output_path)
    if RECTIFY and not os.path.isfile(rect_path):

        os.makedirs(output_path, exist_ok=True)

        if calibration_config_dir is None:
            if checkerboard_input_video_path:
                checkerboard_cap = cv2.VideoCapture(checkerboard_input_video_path)
                point_pairs_list = find_point_pairs(checkerboard_cap)
                subpix_corners_list = subpix_corners(output_path, point_pairs_list, disable_plot=not verbose)
                if verbose:
                    print_corner_count_histogram(point_pairs_list)

                calib_params = calibration_parameters(subpix_corners_list)
                _save_calibration_parameters(calib_params, output_path)
            else:
                logger.error("Either checkerboard input video path or calibration config must be provided.")
                return
        else:
            calib_params = _load_calibration_parameters(calibration_config_dir)
            logger.info(f"Loaded calibration parameters from {calibration_config_dir}.")

        if input_video_path:
            convert_video(
                video_file_path=input_video_path,
                calib_params=calib_params,
                output_file_path=rect_path,
            )
